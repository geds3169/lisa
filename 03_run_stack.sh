...
cleanup_on_failure() {
    ...
    kill "$SUDO_KA_PID" "$INHIBIT_PID" 2>/dev/null
    error "Échec du build/démarrage — nettoyage de la stack L.I.S.A."
    cd "$STACK_DIR" && docker compose down --remove-orphans 2>/dev/null
    docker images --filter "label=lisa.stack=true" -q | xargs docker rmi -f 2>/dev/null
